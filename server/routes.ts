...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // AI Analysis endpoint with complete vehicle data processing
    ...
    try {
      ...
      if (!platform || !lotId || !vin || !vehicleData) {
        return res.status(400).json({
          success: false,
          error: 'Missing required parameters for AI analysis'
        });
      }
      ...
      // Create comprehensive analysis prompt with ALL vehicle data
      const vehicleInfo = `
        COMPLETE VEHICLE ANALYSIS REQUEST
        Platform: ${platform.toUpperCase()}
        Lot ID: ${lotId}
        VIN: ${vin}
        
        VEHICLE SPECIFICATIONS:
        - Year/Make/Model: ${vehicleData.year || 'N/A'} ${vehicleData.make || 'N/A'} ${vehicleData.model || 'N/A'}
        ...
        DAMAGE ASSESSMENT:
        - Primary Damage: ${vehicleData.damage_primary || 'N/A'}
        - Secondary Damage: ${vehicleData.damage_secondary || 'None reported'}
        
        AUCTION DETAILS:
        - Location: ${vehicleData.location || 'N/A'}
        - Title Status: ${vehicleData.title || 'N/A'}
        - Keys Available: ${vehicleData.keys || 'N/A'}
        - Document Type: ${vehicleData.document || 'N/A'}
        - Current Bid: $${vehicleData.current_bid || 0}
        - Reserve Price: $${vehicleData.reserve_price || 0}
        - Auction Date: ${vehicleData.auction_date || 'N/A'}
        - Status: ${vehicleData.status || 'N/A'}
        - Seller: ${vehicleData.seller || 'N/A'}
        
        ADDITIONAL DATA:
        - Images Available: ${vehicleData.images_hd?.length || 0} HD images
        ${vehicleData.vehicle_score ? `- Vehicle Score: ${vehicleData.vehicle_score}` : ''}
        ${vehicleData.salvage_id ? `- Salvage ID: ${vehicleData.salvage_id}` : ''}
        ${vehicleData.iaai_360 ? '- 360° View Available' : ''}
        ${vehicleData.video ? '- Video Available' : ''}
      `;
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert automotive auction analyst specializing in salvage vehicles, damage assessment, and Central America export markets. Provide comprehensive analysis with specific recommendations and confidence scores."
          },
          {
            role: "user",
            content: `${vehicleInfo}
            ...
            1. Market Value Assessment (clean value, post-repair value, damage impact)
            2. Damage Analysis (repair costs, structural concerns)
            3. Export Market Suitability (Central America demand, logistics)
            4. Investment Analysis (total costs, profit potential)
            5. Final Recommendation (BUY/PASS/CAUTION with max bid and confidence score)
            
            Return as JSON format with clear sections and specific dollar amounts.`
          }
        ],
