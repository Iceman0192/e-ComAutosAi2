            ...
            data: {
              ...
              stats: {
                ...
                averagePrice: cachedResult.data.reduce((sum: number, item: any) => 
                  sum + (parseFloat(item.purchase_price) || 0), 0) / Math.max(cachedResult.data.length, 1),
                successRate: 0.75,
                priceTrend: 0.05,
                topLocations: []
              },
        ...
        data: {
          ...
          stats: {
            ...
            averagePrice: (apiResponse.data.data || []).reduce((sum: number, item: any) => 
              sum + (parseFloat(item.purchase_price) || 0), 0) / Math.max((apiResponse.data.data || []).length, 1),
            successRate: 0.75,
            priceTrend: 0.05,
            topLocations: []
          },
